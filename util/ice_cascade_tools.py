...
def netcdf_read_var(name_file, name_var, dim_data):
  '''Read a variable from a netcdf file and check its shape against the supplied
  dimensions.

  name_file = string, netCDF file name or path.
  name_var = string, variable name within name_file
  dim_data = tuple of integers, expected dimensions of the variable'''

  # check for sane inputs
  if not isinstance(name_file, basestring):
    print('Invalid argument: name_file must be a string.')
    return

  if not isinstance(name_var, basestring):
    print('Invalid argument: name_file must be a string.')
    return

  if type(dim_data) is not tuple:
    print('Invalid argument: shape must be a tuple.')
    return

  # read data
  ndim = len(dim_data)
  file = nc.Dataset(name_file, mode = "r") 
  if ndim == 1:
    data = file.variables[name_var][:]
  if ndim == 2:
    data = file.variables[name_var][:,:]
  if ndim == 3:
    data = file.variables[name_var][:,:,:]
  ...
  # check variable size
  if not dim_data == data.shape:
    print('Data dimensions do not match the dimensions specified in dim_data.')
    return
