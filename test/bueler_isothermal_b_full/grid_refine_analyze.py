#!/usr/bin/env python
#
# Analyze results from grid refinement experiment for Test B in [1]. Expects one
...
files = glob.glob(dir+'/bueler_isothermal_b_out_*.nc')
for i in range(len(files)):

  # read data at final timestep only
...
plt.title('Bueler et al 2005, Test B, Mean Absolute Error')
plt.ylabel('Mean absolute error')
plt.savefig(dir+'/bueler_isothermal_b_err_abs_mean.pdf')
plt.close()

# plot max absolute error
plot_data(nx, h_err_abs_max)
plt.title('Bueler et al 2005, Test B, Max Absolute Error')
plt.ylabel('Max absolute error')
plt.savefig(dir+'/bueler_isothermal_b_err_abs_max.pdf')
plt.close()

# plot dome absolute error
plot_data(nx, h_err_abs_dome)
plt.title('Bueler et al 2005, Test B, Dome Absolute Error')
plt.ylabel('Dome absolute error')
plt.savefig(dir+'/bueler_isothermal_b_err_abs_dome.pdf')
plt.close()
...
for i in range(len(h_err)):
  ...
  plt.title('''Bueler et al 2005, Test B
      N = {0:.0f}, Delta = {1:.0f} m'''.format(nx[i], dx[i]))
  plt.savefig(dir+'/bueler_isothermal_b_err_'+str(nx[i]).zfill(d)+'.pdf')
  plt.close()

# create latex report including figures and tabulared scalars
with open(dir+'/bueler_isothermal_b_report.tex', 'w') as f:
  
  f.write('\documentclass[11pt]{article}\n')
  f.write('\usepackage[margin=1.0in]{geometry}\n')
  f.write('\usepackage{graphicx}\n')
  f.write('\usepackage{placeins}\n')
  f.write('\\title{Bueler et al, 2005 Test B, Grid Refinement Experiment}\n')
  ...
  for i in range(len(nx)):
    pdf = dir+'/bueler_isothermal_b_err_'+str(nx[i]).zfill(d)+'.pdf'
  ...
  f.write('\\begin{figure}[h]\n')
  f.write('\centering\n')
  f.write('\centerline{\includegraphics{'+dir+'/bueler_isothermal_b_err_abs_max.pdf}}\n')
  f.write('\end{figure}\n')
  
  f.write('\\begin{figure}[h]\n')
  f.write('\centering\n')
  f.write('\centerline{\includegraphics{'+dir+'/bueler_isothermal_b_err_abs_mean.pdf}}\n')
  f.write('\end{figure}\n')

  f.write('\\begin{figure}[h]\n')
  f.write('\centering\n')
  f.write('\centerline{\includegraphics{'+dir+'/bueler_isothermal_b_err_abs_dome.pdf}}\n')
  f.write('\end{figure}\n')
  
  f.write('\end{document}\n')

# compile report
os.system('pdflatex '+dir+'/bueler_isothermal_b_report.tex')
os.system('pdflatex '+dir+'/bueler_isothermal_b_report.tex')

# clean up working files
os.rename('bueler_isothermal_b_report.pdf',
  dir+'/bueler_isothermal_b_report.pdf')
for f in glob.glob('bueler_isothermal_b_report.*'):
  os.remove(f)
